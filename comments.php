<?php
/*
 * Comment display template of Iconic One.
 * @package WordPress - Themonic Framework
 * @subpackage Iconic_One
 * @since Iconic One 1.0
 */

/*
 * If the current post is protected by a password and
 * the visitor has not yet entered the password we will
 * return early without loading the comments.
 */
if ( post_password_required() )
	return;
?>

<div id="comments" class="comments-area">

	<?php // You can start editing here -- including this comment! ?>

	<?php if ( have_comments() ) : ?>
		<h2 class="comments-title">
			<?php
				printf( _n( 'Ein Gedanke zu &ldquo;%2$s&rdquo;', '%1$s Gedanken zu &ldquo;%2$s&rdquo;', get_comments_number(), 'themonic' ),
					number_format_i18n( get_comments_number() ), '<span>' . get_the_title() . '</span>' );
			?>
		</h2>

		<ol class="commentlist">
			<?php wp_list_comments( array( 'callback' => 'dz_comment', 'style' => 'ol', 'type' => 'comment') ); ?>
			<?php wp_list_comments( array( 'callback' => 'dz_comment', 'style' => 'ol', 'type' => 'pings') ); ?>
		</ol><!-- .commentlist -->

		<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : // are there comments to navigate through ?>
		<nav id="comment-nav-below" class="navigation" role="navigation">
			<h1 class="assistive-text section-heading"><?php _e( 'Comment navigation', 'themonic' ); ?></h1>
			<div class="nav-previous"><?php previous_comments_link( __( '&larr; Older Comments', 'themonic' ) ); ?></div>
			<div class="nav-next"><?php next_comments_link( __( 'Newer Comments &rarr;', 'themonic' ) ); ?></div>
		</nav>
		<?php endif; // check for comment navigation ?>

		<?php
		/* If there are no comments and comments are closed, let's leave a note.
		 * But we only want the note on posts and pages that had comments in the first place.
		 */
		if ( ! comments_open() && get_comments_number() ) : ?>
		<p class="nocomments"><?php _e( 'Comments are closed.' , 'themonic' ); ?></p>
		<?php endif; ?>

	<?php endif; // have_comments() ?>
	
	<?php
	
	ob_start();
	comment_form(array('comment_notes_after' => ''));
	$comment_form = ob_get_clean();
	//insert code to modify $comment_form
	$comment_form = str_replace("h3", "h2", $comment_form);

	echo $comment_form;
	
	?>

</div><!-- #comments .comments-area -->